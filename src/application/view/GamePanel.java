...
public class GamePanel extends JPanel{
	...
	public GamePanel() {
		...
		this.setFont(GameFonts.GAME_PANEL_TEXT_FONT);
		fMText= getFontMetrics(GameFonts.GAME_PANEL_TEXT_FONT);
		wS = new WindowState();
	...
	protected void paintComponent(Graphics g) {
		...
		if(Game.getInstance().isPlaying() || Game.getInstance().isPaused()) {
			...
			g.setFont(GameFonts.GAME_PANEL_GROUND_TEXT_FONT);
			g.drawString("SAFE", 50, 100);
			g.drawString("ZONE", 50, 200);
			g.setFont(GameFonts.GAME_PANEL_TEXT_FONT);
			paintItems(g);
			paintCharacter(g);
			paintMonsters(g);
			paintNPC(g);
	...
	protected void paintInventory(Graphics g) {
		...
		if(inventoryView.getIndexShowItem() != InventoryView.NO_ITEM_SELECTED) {
			...
			if(inventory.get(index).getItem().isConsumable())
				g.drawString("Consumable: " + "Yes" , xInfo + 10, yInfo + 60);
			else
				g.drawString("Consumable: " + "No" , xInfo + 10, yInfo + 60);
		}
	}
	...
	protected void paintCharacter(Graphics g) {
		...
		g.setColor(Color.BLACK);
		g.drawString(character.getLevel().toString(), character.getX() +  4, character.getY() - Settings.DEFAULT_BARS_HEIGTH - 2);		
	...
	protected void paintHelpMenu(Graphics g) {
		g.setFont(GameFonts.GAME_PANEL_TEXT_FONT);
		fMText = getFontMetrics(GameFonts.GAME_PANEL_TEXT_FONT);
		List<String> helpMessages  = Game.getInstance().getHelpSettings().getHelpMessages();
		Character character = Game.getInstance().getCharacter();
	...
	protected void paintItems(Graphics g) {
		//System.out.println(Game.getInstance().getItems().size());
		List<Item> items = Game.getInstance().getItems();
		for(int i=0; i< items.size(); ++i) {
			Item item= items.get(i);
			//System.out.println(item.isDropped());
			if(item.isDropped()) {
	...
	public void paintSaveQuitMenu(Graphics g) {
		g.setFont(GameFonts.GAME_PANEL_BUTTONS_FONT);
		fMText = getFontMetrics(GameFonts.GAME_PANEL_BUTTONS_FONT);
	...
	public void paintReviveMenu(Graphics g) {
		g.setFont(GameFonts.GAME_PANEL_BUTTONS_FONT);
		fMText = getFontMetrics(GameFonts.BUTTONS_FONT);

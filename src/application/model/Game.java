...
public class Game {
	...
	private List<Rectangle> safeZones;
	...
	public void addItemsFromInventory(Inventory inventory) {
		...
		for(int i = 0; i < inventory.size(); ++i) {
			for(int j = 0; j < inventory.get(i).getQuantity(); ++j) {
				...
				items.add(item);
				//System.out.println("Adding an item owned by the player from last save");
			}
		}
	}
	...
	public void dropItem(int position) {
		//System.out.println("Items in the game: " + items.size());
		...
		if(character.getInventory().drop(character.getInventory().get(position-1).getItem())) {
			//System.out.println("Dropping an item (setting dropped in inventory)");
			//trova l'item droppato ed aggiorna il suo stato
			for(int i = 0; i < items.size(); ++i) {
				//System.out.println("Condition: " + items.get(i).getOwnerId() + character.getId());
				if(items.get(i).equals(item) && !items.get(i).isDropped() && items.get(i).getOwnerId() == character.getId()) {
					...
					//System.out.println("Dropping an item (setting dropped in game items)");
					break;
				}
			}
		}
	...
	public List<Rectangle> getSafeZones() {
		return safeZones;
	}

...
public class MenuController implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		//aggiunta di un nuovo profilo
			...
			else if(newProfileName.contains(" ")) {
				JOptionPane.showMessageDialog(mainMenu, "profile name cannot contain spaces");
			}
			else if(newProfileName.length() > Settings.MAX_PROFILE_NAME_LENGTH) {
				JOptionPane.showMessageDialog(mainMenu, "profile name is too long");
			}
			//creazione profilo
			else {
		...
		else if(e.getActionCommand().equals(gameWindow.getMainMenu().getAboutButton().getActionCommand())) {
			gameWindow.switchToPanel("about panel");
		}
		else if(e.getActionCommand().equals(gameWindow.getAboutPanel().getBackToMenuButton().getActionCommand())) {
			switchToMainMenu();
		}
	}
